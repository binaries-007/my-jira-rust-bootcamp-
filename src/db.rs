...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        let reader = io::BufReader::new(File::open(&self.file_path)?);

        let db_state: DBState = serde_json::from_reader(reader).map_err(|err| anyhow!(err))?;

        Ok(db_state)
    }

    fn write_db(&self, db_state: &DBState) -> Result<()> {
        let writer = io::BufWriter::new(File::create(&self.file_path)?);

        serde_json::to_writer_pretty(writer, &db_state).map_err(|err| anyhow!(err))?;
...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        ...
        let latest_id = db_state.last_item_id;

        db_state.epics.insert(latest_id, epic);
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        db_state.stories.insert(latest_id, story);

        db_state
            .epics
            .get_mut(&(epic_id as usize))
            .ok_or_else(|| anyhow!("Unable to get epic with ID: {epic_id}"))?
            .stories
            .push(latest_id);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        let epic = db_state
            .epics
            .remove(&epic_id)
            .ok_or_else(|| anyhow!("Epic with ID: {epic_id} does not exist"))?;

        for story_key in epic.stories {
            db_state.stories.remove_entry(&story_key);
        }
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        let epic_id = epic_id as usize;
        let story_id = story_id as usize;
        let mut db_state = self.read_db()?;

        let stories = &mut db_state
            .epics
            .get_mut(&(epic_id as usize))
            .ok_or_else(|| anyhow!("Could not retrieve Epic with ID: {epic_id}"))?
            .stories;

        for (index, story_index) in stories.iter().enumerate() {
            if story_index == &story_id {
        ...
        db_state
            .stories
            .remove_entry(&story_id)
            .ok_or_else(|| anyhow!("Could not delete story with Story ID: {story_id}"))?;
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<()> {
        ...
        db_state
            .epics
            .get_mut(&epic_id)
            .ok_or_else(|| anyhow!("Could not get Epic with ID: {epic_id}"))?
            .status = status;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        db_state
            .stories
            .get_mut(&story_id)
            .ok_or_else(|| anyhow!("could not get Story with ID: {story_id}"))?
            .status = status;
...
pub mod test_utils {
    ...
    impl Database for MockDB {
        fn read_db(&self) -> Result<DBState> {
            ...
            let db_state = self.last_written_state.borrow().clone();
            Ok(db_state)
        }
...
mod tests {
    ...
    fn create_epic_should_work() {
        ...
        assert_eq!(db_state.last_item_id, expected_id as usize);
        assert_eq!(db_state.epics.get(&(id as usize)), Some(&epic));
    }
    ...
    fn create_story_should_work() {
        ...
        assert_eq!(
            db_state
                .epics
                .get(&(epic_id as usize))
                .unwrap()
                .stories
                .contains(&(id as usize)),
            true
        );
        assert_eq!(db_state.stories.get(&(id as usize)), Some(&story));
    }
    ...
    fn delete_epic_should_work() {
        ...
        assert_eq!(db_state.epics.get(&(epic_id as usize)), None);
        assert_eq!(db_state.stories.get(&(story_id as usize)), None);
    }
    ...
    fn update_epic_status_should_work() {
        ...
        assert_eq!(
            db_state.epics.get(&(epic_id as usize)).unwrap().status,
            Status::Closed
        );
    }
    ...
    fn update_story_status_should_work() {
        ...
        assert_eq!(
            db_state.stories.get(&(story_id as usize)).unwrap().status,
            Status::Closed
        );
    }
    ...
    mod database {
        ...
        fn write_db_should_work() {
            ...
            let write_result = db.write_db(&db_state);
